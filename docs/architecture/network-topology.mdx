...
本頁說明 GPM 派車系統在現場的**網路連線拓樸**，對應 **MCS → AGVS → AGV** 三層架構，並描述各服務與裝置之間的通訊協定。
...
| SECS Middleware ↔ AGVS | **RESTful API** | 任務轉送、事件與上報資料交換 |
| AGVS ↔ VMS | **RESTful API** | 派車請求、任務進度與狀態同步 |
| VMS ↔ AGV | **RESTful API** | 任務下達、狀態／導航事件上報 |
| AGVS ↔ 生產裝置 | **CC-Link / Modbus/TCP / MC Protocol** | 讀寫裝置狀態、Port 訊號 |
| AGVS ↔ 充電站 | **TCP/IP / PMBus** | 充電站狀態監控與充電控制 |

:::info 邏輯 vs 實體
上表為**應用層通訊協定**。AGV 與 VMS 的 RESTful API 在實體網路上通常仍經由 **乙太網路／Wi-Fi** 承載；生產裝置與充電站依現場配置接入 Switch 或專用通訊模組。
:::
...
SECS Middleware 負責 SECS 訊息與 AGVS 內部格式的轉換，不直接與 AGV 或現場裝置通訊。
...
| AGVS → SECS Middleware | 推送 AGVS 事件與上報資料，供轉拋至 MCS |
...
**AGVS 核心服務** 直接與現場**生產裝置**（Load Port、Process Tool、Buffer 等）通訊，取得裝置狀態並協調派車時機。支援以下協定：

| 協定 | 適用場景 |
|------|----------|
| **CC-Link** | 三菱 PLC 為主的裝置網路 |
| **Modbus/TCP** | 通用工業 PLC，讀寫暫存器對映表 |
...
- 寫入派車允許、任務完成確認等控制位
- 依裝置型別選擇對應協定與暫存器／位址對映表

:::tip 協定選擇
同一現場可混用多種協定：依各機臺 PLC 品牌與客戶規格，在 AGVS 裝置管理模組中分別配置 CC-Link、Modbus/TCP 或 MC Protocol 連線。
:::
...
3. **裝置網段**：生產裝置（CC-Link / Modbus/TCP / MC Protocol）與 AGVS 之間規劃專用網段或 Gateway，避免廣播風暴。
